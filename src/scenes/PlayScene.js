class PlayScene extends Scene {
  ...
  initLevel() {
    ...
    const userEnv = {};
    userEnv.killPlayer = (data) => {
      this.callMethod('killplayer', data);
    };
    userEnv.destroyObject = (data) => {
      this.callMethod('destroyobject', data);
    };
    userEnv.setVelocity = (data) => {
      this.callMethod('setvelocity', data);
    };
    userEnv.addVelocity = (data) => {
      this.callMethod('addvelocity', data);
    };
    userEnv.setPosition = (data) => {
      this.callMethod('setposition', data);
    };
    userEnv.addPosition = (data) => {
      this.callMethod('addposition', data);
    };
    for (const gadgetBody of this.gadgetBodies) {
      gadgetBody.env = userEnv;
    }
    for (const gadgetBody of this.gadgetBodies) {
  ...
  callUserCallback(gadgetBody, event, data) {
    if (gadgetBody.callback) {
      ...
      // }
      try {
        gadgetBody.callback(event, data, gadgetBody.env);
      }
      catch (e) {
    ...
    try {
      ...
      switch (method.toLowerCase()) {
        ...
        case 'destroyobject': {
          ...
          body.destroy();
          this.gadgetBodies.splice(this.gadgetBodies.indexOf(body), 1);
          break;
        }
        case 'setvelocity': {
          const body = getBodyByName(data.name);
          body.linearVelocity.copy({ x: data.x, y: data.y });
          break;
        }
        case 'addvelocity': {
          const body = getBodyByName(data.name);
