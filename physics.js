  ...
  step(deltaTime) {
    ...
    for (let colliderNode1 = this.colliders.first; colliderNode1 != null; colliderNode1 = colliderNode1.next) {
      ...
      for (let colliderNode2 = colliderNode1.next; colliderNode2 != null; colliderNode2 = colliderNode2.next) {
        ...
        collider1.body?.onPhysicsCollision?.(collider2.body, collision.collision.point);
        collider2.body?.onPhysicsCollision?.(collider1.body, collision.collision.point);
      }
    }
    for (const {collider1, collider2, collision} of collisions) {
      ...
      const normalInverseMass1 = body1.inverseLinearMass + body1.inverseAngularMass * Vector.dot(collision.normal, tangent1) ** 2;
      const normalInverseMass2 = body2.inverseLinearMass + body2.inverseAngularMass * Vector.dot(collision.normal, tangent2) ** 2;
      ...
      body1.angularVelocity -= Vector.dot(collision.normal, tangent1) * collisionImpulse * body1.inverseAngularMass;
      body2.angularVelocity += Vector.dot(collision.normal, tangent2) * collisionImpulse * body2.inverseAngularMass;
      ...
      const tangentInverseMass1 = body1.inverseLinearMass + body1.inverseAngularMass * Vector.dot(collisionTangent, tangent1) ** 2;
      const tangentInverseMass2 = body2.inverseLinearMass + body2.inverseAngularMass * Vector.dot(collisionTangent, tangent2) ** 2;
      ...
      body1.angularVelocity -= Vector.dot(collisionTangent, tangent1) * frictionInpulse * body1.inverseAngularMass;
      body2.angularVelocity += Vector.dot(collisionTangent, tangent2) * frictionInpulse * body2.inverseAngularMass;
    }
    for (const {collider1, collider2, collision} of collisions) {
      const body1 = collider1.body;
      const body2 = collider2.body;
      ...
      const inverseMass1 = body1.inverseLinearMass + body1.inverseAngularMass * Vector.dot(collision.normal, tangent1) ** 2;
      const inverseMass2 = body2.inverseLinearMass + body2.inverseAngularMass * Vector.dot(collision.normal, tangent2) ** 2;
      correctionInpulse /= inverseMass1 + inverseMass2;
      body1.applyCorrectionImpulse(collision.point, Vector.multiply(collision.normal, -correctionInpulse));
      body2.applyCorrectionImpulse(collision.point, Vector.multiply(collision.normal, correctionInpulse));
    }
    for (const body of this.bodies) {
...
class PhysicsBody {
  ...
  applyCorrectionImpulse(point, impulse) {
    ...
    this.angularVelocityCorrection += Vector.cross(point.clone().subtract(this.position), impulse) * this.inverseAngularMass;
  }
